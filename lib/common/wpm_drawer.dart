...
class WPMDrawerLoader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Stream<List<Property>> propertiesStream =
        AppStateProvider.of(context).propertiesStream;
...
class _WPMDrawerViewState extends State<_WPMDrawerView> {
  static const int extraTileCount = 2;
  ...
  Widget build(BuildContext context) {
    ...
    final Stream<Property> selectedPropertyStream =
        appState.selectedPropertyStream;
      ...
      builder: (
        ...
        return new Drawer(
          child: new ListView.builder(
              ...
              itemCount:
                  _showSignOut ? 2 : widget.properties.length + extraTileCount,
              itemBuilder: (BuildContext context, int index) {
                /* DRAWER HEADER index: 0 */
                if (index == 0) {
                  return new DrawerHeader(
                      user: user, toggleShowSignOut: _toggleShowSignOut);
                }
                if (_showSignOut) {
...
//                  }
                ...
                final Property property =
                    widget.properties[index - extraTileCount];
                print('selectedProperty.id=[${selectedProperty
...
class DrawerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => new UserAccountsDrawerHeader(
        ...
        currentAccountPicture: new CircleAvatar(
          backgroundImage:
              user.photoUrl != null ? new NetworkImage(user.photoUrl) : null,
